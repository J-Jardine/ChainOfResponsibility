 ...
 * @author t7077222
 */
public class LargeCarCourier implements VehicleChain {
        ...
        if (vehicle.getOrderWeight() < 150) {
            if (isFull == false) {
                if (vehicleCount < maxVehicle) {
                    increment();
                    System.out.println("A Large Car With: " + vehicle.toString()
                            + " ,Capacity [" + vehicleCount
                            + "/" + maxVehicle + "]");
                } else {
                    isFull = true;
                    System.out.println("All Of The Large Cars Are In Use Passing The Request To The Next Available Vehicle!");
                }
            }
        } else {
