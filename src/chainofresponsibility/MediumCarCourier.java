...
/**
 * Medium car vehicle handler
 *
 * @author t7077222
 */
public class MediumCarCourier implements VehicleChain {
    ...
    public boolean vehicleChain(Vehicle vehicle) {
        ...
        if (vehicle.getOrderWeight() < 100) {
            if (isFull == false) {
                if (vehicleCount < maxVehicle) {
                    increment();
                    System.out.println("A Medium Car With: " + vehicle.toString()
                            + " ,Capacity [" + vehicleCount
                            + "/" + maxVehicle + "]");
                } else {
                    isFull = true;
                    System.out.println("All Of The Medium Cars Are In Use Passing The Request To The Next Available Vehicle!");
