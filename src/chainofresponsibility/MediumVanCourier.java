 ...
 * @author t7077222
 */
public class MediumVanCourier implements VehicleChain {
    ...
    public boolean vehicleChain(Vehicle vehicle) {
        ...
        if (vehicle.getOrderWeight() < 400) {
            if (isFull == false) {
                if (vehicleCount < maxVehicle) {
                    increment();
                    System.out.println("A Medium Van With: " + vehicle.toString()
                            + " ,Capacity [" + vehicleCount
                            + "/" + maxVehicle + "]");
                } else {
                    isFull = true;
                    System.out.println("All Of The Medium Vans Are In Use Passing The Request To The Next Available Vehicle!");
                }
            }
        } else {
